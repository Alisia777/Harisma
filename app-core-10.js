...
async function applySkuDecisionApproval(decision, comment = '') {
  ...
  if (decision.type === 'PRODUCT_STATUS_CHANGE') {
    if (decision.payload?.clearOverride) {
      await removeProductLifecycleStatus(decision.articleKey);
    } else {
...
async function init() {
  ...
  try {
    ...
    const localStorageSnapshot = {
      ...
      productLifecycleOverrides: Array.isArray(local.productLifecycleOverrides) ? local.productLifecycleOverrides : [],
      productLifecycleOverrideDeletes: Array.isArray(local.productLifecycleOverrideDeletes) ? local.productLifecycleOverrideDeletes.map(normalizeRepricerDeleteTombstone).filter((item) => item.articleKey) : [],
      skuDecisionApprovals: Array.isArray(local.skuDecisionApprovals) ? local.skuDecisionApprovals.filter((item) => item && typeof item === 'object') : [],
