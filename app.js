(function () {
  ...
  function v76MergeRemoteCollection(localRows, remoteRows, normalizer) {
    const merged = new Map();
    for (const row of localRows || []) {
      const item = normalizer(row);
  ...
  async function v76PullRemoteOnly() {
    ...
    try {
      ...
      const remoteProducts = (products || []).map(v76FromRemoteProduct);
      const remoteNovelties = (novelties || []).map(v76FromRemoteNovelty);
      const remoteNotebook = (notebook || []).map(v76FromRemoteNotebook).sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1));

      // Не затираем локальный список пустым ответом из одной из remote-таблиц.
      // Иначе после изменения статуса товара могли пропасть все новинки или вся матрица.
      state.v76.store.products = v76MergeRemoteCollection(state.v76.store.products, remoteProducts, v76NormalizeProduct)
        .sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'ru'));
      state.v76.store.novelties = v76MergeRemoteCollection(state.v76.store.novelties, remoteNovelties, v76NormalizeNovelty)
        .sort((a, b) => v76MonthKey(a.launchMonth).localeCompare(v76MonthKey(b.launchMonth)) || String(a.name || '').localeCompare(String(b.name || ''), 'ru'));
      state.v76.store.notebook = v76MergeRemoteCollection(state.v76.store.notebook, remoteNotebook, v76NormalizeNotebook)
        .sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1));
      v76SaveLocalStore();

      state.v76.remote.lastSyncAt = v76Now();
      state.v76.remote.note = (remoteProducts.length || remoteNovelties.length || remoteNotebook.length)
        ? `Блок Ксении синхронизирован · ${fmt.date(state.v76.remote.lastSyncAt)}`
        : 'Remote пустой — оставлен локальный список товаров и новинок';
