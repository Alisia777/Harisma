(function () {
  ...
  var VIEW_GUIDES = {
    ...
    repricer: {
      title: 'Репрайсер',
      summary: 'Рабочий контур цены: сначала закрываем красные причины, затем проверяем совет и только после этого выгружаем цену.',
      watch: [
        'Статус товара, свежесть текущей цены, индивидуальная маржа SKU, MIN/MAX и причина рекомендации.',
        'Верхний индикатор: СТОП означает, что файл цен пока нельзя отправлять на площадку.'
      ],
      actions: [
        'Маржа: нажмите «Скачать маржу SKU», заполните «Новая маржа SKU, %», сохраните .xls и нажмите «Загрузить маржу SKU».',
        'MIN/MAX, статус или точечная цена: используйте «Аудит в Excel», заполните только новые значения и загрузите аудит обратно.',
        'Шаблон WB/Ozon скачивайте только когда строки зелёные и портал показывает, что выгрузка безопасна.'
      ],
      avoid: [
        'Не заполняйте колонки текущих значений — редактируются только колонки, начинающиеся с «Новая» или «Новый».',
        'Не отправляйте пустой или красный шаблон на маркетплейс.',
        'Статус и изменение цены на 10% и более не обходят РОП: дождитесь подтверждения задачи.'
      ],
      workflow: 'Проверить дату и СТОП -> закрыть маржу/MIN/MAX -> загрузить исправления -> проверить задачи РОП -> убедиться, что строки зелёные -> скачать шаблон WB/Ozon -> загрузить его на площадку.'
    },
    prices: {
      title: 'Цены',
