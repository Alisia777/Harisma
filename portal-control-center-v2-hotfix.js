(function () {
  ...
  function renderGeneralTaskEnhancements(root) {
    ...
    const owners = ownerOptions();
    const recentHistory = getRecentTaskHistory(tasks, 6);
    const historyHtml = recentHistory.length
      ? recentHistory.map((item) => renderTaskHistoryItem(item)).join('')
      : '<div class="empty">Пока нет истории по задачам. Первый комментарий или отчёт появится здесь.</div>';

    const target = root.querySelector('.kpi-strip');
    if (!target) return;

    target.insertAdjacentHTML('afterend', `
      <div class="two-col" style="margin:14px 0" data-control-center-v2-block>
        <div class="card">
          <div class="section-subhead">
            <div>
              <h3>Поставить общую задачу</h3>
              <p class="small muted">Для сотрудника, РОПа или межкомандной задачи без привязки к одному SKU.</p>
            </div>
          ...
          <form id="generalTaskForm" class="form-grid compact">
            ...
            <input name="entityLabel" placeholder="Проект / тема / блок">
            <select name="platform">
        ...
        <div class="card">
          <div class="section-subhead">
            <div>
              <h3>История по задачам</h3>
              <p class="small muted">Здесь видно, кто обновил задачу, что уже сделано и где есть отчёт по результату.</p>
            </div>
            ${badge(`${fmt.int(recentHistory.length)} записей`, recentHistory.length ? 'info' : 'ok')}
          </div>
          <div class="decision-list">${historyHtml}</div>
        </div>
      </div>
    `);
  ...
  function installGlobalBindings() {
    ...
    window.closeTaskWithReport = closeTaskWithReport;
