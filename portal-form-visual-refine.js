(function () {
  ...
  const normPlatform = (v) => ['wb', 'ozon', 'retail', 'cross'].includes(String(v || '').toLowerCase()) ? String(v).toLowerCase() : 'cross';
        ...
        <form id="generalTaskForm" class="form-stack">
          <div class="ui-group">
            ...
            <div class="ui-grid-3">
              ...
              ${fixed ? `<div class="ui-note"><strong>Контур задачи</strong>${escapeHtml(meta(platform).label)}<input type="hidden" name="platform" value="${escapeHtml(platform)}"></div>` : `<label class="ui-field"><span class="ui-label">Контур</span><select name="platform"><option value="cross" ${platform === 'cross' ? 'selected' : ''}>Общий контур</option><option value="wb">РОП WB</option><option value="ozon">РОП Ozon</option><option value="retail">ЯМ / сети</option></select></label>`}
            </div>
  ...
  function controlRefined() {
    ...
    const cards = cardsRoot ? Array.from(cardsRoot.children).filter((node) => node.classList?.contains('card')) : [];
    if (cards[1]) cards[1].outerHTML = generalCard(selected, owners(), tasks.filter((item) => ['waiting_rop', 'waiting_decision'].includes(item.status)).length);
