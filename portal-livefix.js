(function () {
    ...
    if (typeof window.__alteaLoadPortalSnapshot === 'function') {
      try {
        ...
        if (snapshot !== null && snapshot !== undefined) {
          cache[key] = snapshot;
          const app = appState();
          if (app && !app[key]) app[key] = snapshot;
          return snapshot;
        }
      } catch (error) {
        console.warn(`[portal-livefix] snapshot ${path}`, error);
      }
    }
  ...
  function normalizeSyncBadge() {
    ...
    if (app.team.mode !== 'error' && !/Ошибка|Supabase|база пока без решений|ценовой контур/i.test(note)) return;
    app.team.mode = 'local';
    app.team.ready = false;
    app.team.note = /ценовой контур|база пока без решений/i.test(note)
      ? 'Локальный режим · ценовой контур пока без решений'
      : 'Локальный режим · командная база недоступна';
  ...
  function applyPulse() {
    ...
    if (!root) return;
    if (root.querySelector('[data-portal-ui-hotfix-hero]')) {
