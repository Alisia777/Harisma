(function () {
  ...
  const GUIDE_TEXT = {
    topActions: [
      ['1. Скачать рабочий Excel', 'Скачивает один файл для маржи, MIN/MAX и остальных изменений по SKU. Ничего не меняет, пока файл не загружен обратно.'],
      ['2. Загрузить заполненный Excel', 'Принимает тот же рабочий файл после заполнения левых колонок и создаёт необходимые согласования.']
    ],
    settingsActions: [
  ...
  function enhanceTopActionGuide(root) {
    ...
    const guide = buildGuide(
      'top-actions',
      'Один рабочий Excel',
      'Порядок один: скачать файл, заполнить левые колонки и загрузить этот же файл обратно. На маркетплейс данные автоматически не отправляются.',
  ...
  function normalizeVisibleLabels(root) {
    const buttonMap = new Map([
      ['Audit Excel', '1. Скачать рабочий Excel'],
      ['WB загрузка', 'WB цены'],
