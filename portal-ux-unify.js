(function () {
  ...
  var VIEW_META = {
    ...
    launches: {
      ...
      title: "Завести новинку, назначить owner, связать SKU и довести запуск до READY.",
      text: "Рабочее место продукта: сначала закрываем owner, SKU, материалы и статус, а полный календарь открываем только когда нужен.",
      chips: ["Добавить новинку", "Статус READY", "Gantt по кнопке"]
    },
    "launch-control": {
  ...
  function rewriteViewCopy(root, viewKey) {
    ...
    if (viewKey === "launches") {
      ...
      setText(root, ".section-title p", "Рабочее место директора по продукту: завести новинку, назначить owner, связать SKU, закрыть материалы и довести запуск до READY.");
      return;
    }
