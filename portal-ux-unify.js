(function () {
  ...
  var VIEW_META = {
    ...
    launches: {
      eyebrow: "Новинки",
      title: "Календарь, проекты товаров и задачи запуска в одном рабочем слое.",
      text: "Карточка новинки открывается как папка товара: поставщик, SKU, календарь, комментарии и задачи этапов остаются вместе.",
      chips: ["Календарь", "Проекты", "Задачи"]
    },
    "launch-control": {
    ...
    "product-leaderboard": {
      ...
      chips: ["BI-срез", "План-факт", "Ответственные"]
    },
    prices: {
  ...
  function rewriteViewCopy(root, viewKey) {
    ...
    if (viewKey === "launches") {
      ...
      setText(root, ".section-title p", "Единая доска новинок: календарь, карточки проектов, папки товаров и задачи запуска.");
      return;
    }
