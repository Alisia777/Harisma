...
async function run() {
  ...
  try {
    ...
    ['index.html', 'live-index.html', 'docs/index.html'].forEach((fileName) => {
      ...
      assert.ok(html.includes('app-core-01.js?v=20260728policyworkflow3'), `${fileName} must cache-bust OOS loading`);
      ...
      assert.ok(html.includes('portal-snapshot-refresh-hotfix.js?v=20260727snapshotretry1'), `${fileName} must cache-bust OOS refresh scope`);
    });
