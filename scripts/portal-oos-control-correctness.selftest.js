...
async function run() {
  ...
  try {
    ...
    ['index.html', 'live-index.html', 'docs/index.html'].forEach((fileName) => {
      ...
      assert.ok(html.includes('app-core-01.js?v=20260724ooscorrectness2'), `${fileName} must cache-bust OOS loading`);
      assert.ok(html.includes('styles.css?v=20260724oosforecast1'), `${fileName} must cache-bust OOS forecast styles`);
      ...
      assert.ok(html.includes('app-core-11.js?v=20260724planfactcorrectness3oosforecast1scenario1'), `${fileName} must cache-bust OOS logic without discarding the plan-fact correctness cache version`);
      assert.ok(html.includes('portal-route-layer-lock.js?v=20260724oosforecast1'), `${fileName} must cache-bust the route lock`);
      assert.ok(html.includes('portal-snapshot-refresh-hotfix.js?v=20260724ooscorrectness2'), `${fileName} must cache-bust OOS refresh scope`);
    });
