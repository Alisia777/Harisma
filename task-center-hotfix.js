(function () {
  ...
  function workstreamKey(task, sku) {
  ...
  function buildFilteredTasks(options) {
    ...
    return getAllTasks().filter((task) => {
      ...
      const meta = workstreamMeta(workstreamKey(task, sku));
      const hay = [
        task.title,
        task.nextAction,
        task.reason,
  ...
  function buildWorkstreamSummary(tasks, key) {
    const grouped = key === 'all'
      ? sortTasks(tasks)
      : sortTasks(tasks.filter((task) => workstreamKey(task, getSku(task.articleKey)) === key));
  ...
  function patchSkuModalPlatformOptions() {
    if (typeof renderSkuModal !== 'function') return;
    const original = renderSkuModal;
    renderSkuModal = function patchedRenderSkuModal(articleKey) {
      original(articleKey);
  ...
  taskPlatformBadge = function patchedTaskPlatformBadge(task) {
    const meta = workstreamMeta(workstreamKey(task, getSku(task.articleKey)));
    return badge(meta.chip, meta.kind);
  };

  filteredControlTasks = function patchedFilteredControlTasks(options) {
    return buildFilteredTasks(options);
  };
  ...
  window.setTimeout(() => {
    try {
      ...
      console.warn('[task-center-hotfix]', error);
    }
  }, 150);
})();